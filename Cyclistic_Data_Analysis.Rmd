...
```{r Format data types for calculations }
...
avg_ride_length <- bike_rides_2021$ride_length_num %>% 
  mean() 

max_ride_length <- bike_rides_2021$ride_length_num %>% 
  max() 

min_ride_length <- bike_rides_2021$ride_length_num %>% 
  min() 

```

```{r Derive more insights }

# bike rides with less than 5 mins 
bike_rides_less_then_5mins <- bike_rides_2021 %>%  filter(ride_length_num <300 )
...
bike_rides_2021 %>%  filter(ride_length < 100 & start_lat > 41.9)
ggplot(data = bike_rides_2021)+
  geom_bar(mapping = aes(x= bike_rides_2021$member_casual, color = bike_rides_2021$rideable_type))
